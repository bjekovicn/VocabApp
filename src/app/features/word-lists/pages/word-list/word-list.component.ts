...
export class WordListComponent {
  ...
  public readonly filteredWords = computed(() => {
    ...
    // Reči sa pokušajima sortirane po slabosti (najgore prve), neprobane na kraju
    return [...words].sort((a, b) => {
      const aAttempts = this.getTotalAttempts(a);
      const bAttempts = this.getTotalAttempts(b);
      if (aAttempts === 0 && bAttempts === 0) return 0;
      if (aAttempts === 0) return 1; // a ide na kraj
      if (bAttempts === 0) return -1; // b ide na kraj
  ...
  /** Ukupan broj tačnih kroz sva 4 modusa */
  ...
  /** Ukupan broj netačnih kroz sva 4 modusa */
  ...
  /** Ukupna tačnost kroz sva 4 modusa */
  ...
  /** Skor slabosti — veći = lošije, koristimo za sortiranje */
